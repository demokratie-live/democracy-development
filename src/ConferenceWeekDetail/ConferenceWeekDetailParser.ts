...
namespace Parser {
  ...
  export class ConferenceWeekDetailParser
    implements IParser<ConferenceWeekDetailsData, ConferenceWeekDetailsMeta> {
    public async parse(
      ...
      while ((m = regex_DateSession.exec(string)) !== null) {
        ...
        m.forEach((match, group) => {
          if (group === 1) {
            ...
            session.date = moment
              .utc(session.dateText, "DD MMM YYYY", "de")
              .toDate();
              ...
              n.forEach((match, group) => {
                if (group === 1) {
                  ...
                  if (
                    top.time &&
                    lastTopTime &&
                    lastTopTime.getUTCHours() > top.time.getUTCHours()
                  ) {
                    newDay = true;
                  }
                ...
                if (group === 3) {
                  ...
                  topicLines.forEach((line) => {
                    if (
                      topicPart.lines.length !== 0 &&
                      (line === "" || line.match(regex_newTopicPart) !== null)
                    ) {
