...
export interface DeputyFunction {
  category: string;
  functions: string[];
}

export interface DeputyProfileResult {
  id: string;
  imgURL: string;
  imgCopyright: string;
  name: string;
...
export class DeputyProfileParser implements IParser<DeputyProfileData, DeputyProfileMeta> {
  ...
  public async parse(data: DataPackage<DeputyProfileData, DeputyProfileMeta>): Promise<DataPackage<object, object>> {
    ...
    let imgURL: string = '';
    let name: string = '';
    const regex_imgURL_name =
      /<div class="bt-bild-standard[\s\S]*?">[\s\S]*?<img[\s\S]*?data-img-md-normal="([\s\S]*?)"[\s\S]*?title="([\s\S]*?)[,"][\s\S]*?>[\s\S]*?<span class="bt-bild-info-icon">/gm;
    ...
    const regex_biography_sel =
      /<h4>Biografie<\/h4>[\s\S]*?<div class="bt-collapse-padding-bottom">[\s\S]*?<div>([\s\S]*?)<\/div>/gm;
    ...
    let functions: DeputyFunction[] = [];
    const regex_functions_sel =
      /<div id="bt-aemter-collapse"[\s\S]*?>([\s\S]*?)<h4>Veröffentlichungspflichtige Angaben<\/h4>/gm;
    ...
    functions_raw.forEach((functions_functions_raw) => {
      let f: DeputyFunction = { category: '', functions: [] };
    ...
    const regex_speechesURL =
      /<input type="hidden" data-fid="([\s\S]*?)" data-fpropertyfield="([\s\S]*?)" data-value="([\s\S]*?)"\/>[\s\S]*?data-url="([\s\S]*?)"/gm;
    ...
    const speechesURL: string =
      base_url + speechesURL4 + '?' + speechesURL2 + '=' + speechesURL1 + '%23' + speechesURL3;

    // VotesURL
    let votesURL: string = '';
    const regex_votesURL =
      /<div class="bt-abstimmungen-show-more bt-abstimmungen-showall">[\s\S]*?<button class="btn loadMore" type="submit" data-url="([\s\S]*?)">/gm;
    ...
    let publicationRequirement: string[] = [];
    const publicationRequirement_sel_regex =
      /<h4>Veröffentlichungspflichtige Angaben<\/h4>[\s\S]*?<ul[\s\S]*?>([\s\S]*?)<\/ul>/gm;
    ...
    const result: DeputyProfileResult = {
      id,
      imgURL,
      imgCopyright,
      name,
...
export default DeputyProfileParser;
