...
export class NamedPollParser implements IParser<NamedPollData, NamedPollMeta> {
  ...
  public async parse(data: DataPackage<NamedPollData, NamedPollMeta>): Promise<DataPackage<object, object>> {
    ...
    const regex_dateTitleDescription =
      /<article [\s\S]*?>[\s\S]*?<h3 class="[\s\S]*?">[\s\S]*?<span class="bt-dachzeile">([\s\S]*?)<\/span>[\s\S]*?<br\/>([\s\S]*?)<\/h3>[\s\S]*?<p>([\s\S]*?)<\/p>[\s\S]*?<\/article>/gm;
    ...
    let resultAll: {
      ...
      abstain: number | null;
      na: number | null;
    } = { total: null, yes: null, no: null, abstain: null, na: null };
    let resultAll_sel: string = '';
    const regex_membersVotedResultAll =
      /<h3>Gesamtergebnis, (.*?) Mitglieder<\/h3>[\s\S]*?<ul class="bt-chart-legend">([\s\S]*?)<\/ul>/gm;
    const regex_ResultAll =
      /<li class="bt-legend-ja"><span>(.*?)<\/span>[\s\S]*?<li class="bt-legend-nein"><span>(.*?)<\/span>[\s\S]*?<li class="bt-legend-enthalten"><span>(.*?)<\/span>[\s\S]*?<li class="bt-legend-na"><span>(.*?)<\/span>[\s\S]*?/gm;
    ...
    const regex_partyVotes =
      /<div class="bt-teaser-chart-solo" data-value="(.*)?">[\s\S]*?<h4 class="bt-chart-fraktion">[\s\S]*?<span>(.*?)Mitglieder<\/span>[\s\S]*?<\/h4>[\s\S]*?<div class="bt-teaser-text-chart">[\s\S]*?<ul class="bt-chart-legend">([\s\S]*?)<\/ul>/gm;
    const regex_partyVoteResults =
      /<li class="bt-legend-ja">(.*?) .*?<\/li>[\s\S]*?<li class="bt-legend-nein">(.*?) .*?<\/li>[\s\S]*?<li class="bt-legend-enthalten">(.*?) .*?<\/li>[\s\S]*?<li class="bt-legend-na">(.*?) .*?<\/li>/gm;
    while ((m = regex_partyVotes.exec(string)) !== null) {
      ...
      } = { name: null, votes: { total: null, yes: null, no: null, abstain: null, na: null } };
      // The result can be accessed through the `m`-variable.
      m.forEach((match, group) => {
        if (group === 1) {
    ...
    const regex_plenarProtocolURL =
      /data-url="(.*?)"[\s\S]*?><i class="icon-doc">[\s\S]*?Plenarprotokoll[\s\S]*?<\/a>/gm;
    ...
    const regex_videoURLs =
      /<input id="video.*?"[\s\S]*?data-url="(.*?)"[\s\S]*?>[\s\S]*?<label[\s\S]*?>(.*?)<\/label>/gm;
    ...
    let topTitle: string = '';
    const regex_detailedDescription =
      /<i class="icon-docs"><\/i><\/button>[\s\S]*?<\/div>[\s\S]*?<\/fieldset>[\s\S]*?<\/form>[\s\S]*?<\/div>[\s\S]*?<\/div>[\s\S]*?<\/div>[\s\S]*?<\/div>[\s\S]*?<\/div>[\s\S]*?<h4 class="[\s\S]*?">([\s\S]*?)<\/h4>[\s\S]*?<p>([\s\S]*?)<\/p>/gm;
    ...
    let speeches: Array<{
      ...
      deputyImgURL: string | null;
      mediathekURL: string | null;
      function: string | null;
      party: string | null;
    }> = [];
    const regex_speeches =
      /<div class="col-xs-4 col-sm-3 col-md-6 bt-teaser">[\s\S]*?<a title="(.*?)" href="(.*?)"[\s\S]*?<img[\s\S]*?data-img-md-normal="(.*?)"[\s\S]*?class="img-responsive"\/>[\s\S]*?<h3>(.*?)<\/h3>[\s\S]*?<p class="bt-person-(.*?)">(.*?)<\/p>[\s\S]*?<\/div>[\s\S]*?<\/div>[\s\S]*?<\/a>[\s\S]*?<\/div>/gm;
    while ((m = regex_speeches.exec(string)) !== null) {
      ...
      let speech: {
        ...
        function: string | null;
        party: string | null;
      } = { deputyName: null, deputyImgURL: null, mediathekURL: null, function: null, party: null };
    ...
    return new DataPackage<object, object>(
      {
        ...
        plenarProtocolURL,
        media: { iTunesURL, mediathekURL, videoURLs },
        speeches,
      },
      data.meta,
    );
...
export default NamedPollParser;
