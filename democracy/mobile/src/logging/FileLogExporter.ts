...
export class FileLogExporter implements LogRecordExporter {
  private config: FileLogExporterConfig;
   ...
   */
  private formatLogRecord(record: ReadableLogRecord): string | null {
    try {
      ...
      const jsonRecord: JsonLineLogRecord = {
        ts: Date.now(),
  ...
  private async cleanupOldFiles(): Promise<void> {
    try {
      ...
      const logFiles = files.filter(file => 
        file.name.endsWith('.jsonl') && file.name.startsWith('app-logs-')
      );
